export const CATEGORIES = [
  { id: 'kanji', label: 'Kanji', color: '#FF4B1F', cssVar: 'var(--kanji)', num: '01', weight: 1.20, unit: 'karakter', defaultTarget: 15 },
  { id: 'bunpou', label: 'Bunpou', color: '#0047AB', cssVar: 'var(--bunpou)', num: '02', weight: 1.15, unit: 'pola tata bahasa', defaultTarget: 10 },
  { id: 'vocab', label: 'Vocab', color: '#1A8A3E', cssVar: 'var(--vocab)', num: '03', weight: 1.00, unit: 'kosakata baru', defaultTarget: 30 },
  { id: 'listening', label: 'Listening', color: '#B8860B', cssVar: 'var(--listening)', num: '04', weight: 0.90, unit: 'menit audio/video', defaultTarget: 25 },
];
 ...
 * 2. Smooth Logarithmic Diminishing Returns: V_eff = 160 * (1 - e^(-V/120)) + 0.18 * V
...
export function computeScore(entry) {
  ...
  // Smooth sublinear saturation
  const saturatedVolume = 160 * (1 - Math.exp(-weightedVolume / 120)) + 0.18 * weightedVolume;
...
export function computeScoreBreakdown(entry) {
  ...
  const saturatedVolume = weightedVolume > 0
    ? 160 * (1 - Math.exp(-weightedVolume / 120)) + 0.18 * weightedVolume
    : 0;
...
export const MASTERY_TIERS = [
  { id: 'ronin', title: '浪人 Rōnin', minMomentum: 0, maxMomentum: 35, color: '#6B6658', desc: 'Memulai perjalanan & membentuk kedisiplinan dasar.' },
  { id: 'shugyosha', title: '修行者 Shugyōsha', minMomentum: 35, maxMomentum: 80, color: '#0047AB', desc: 'Disiplin belajar konsisten setiap hari.' },
  { id: 'tatsujin', title: '達人 Tatsujin', minMomentum: 80, maxMomentum: 140, color: '#1A8A3E', desc: 'Penguasaan tangguh dan momentum belajar tinggi.' },
  { id: 'meijin', title: '名人 Meijin', minMomentum: 140, maxMomentum: 9999, color: '#FF4B1F', desc: 'Tingkat tertinggi — master grind immersion!' },
];
